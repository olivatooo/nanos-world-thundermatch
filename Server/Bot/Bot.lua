...
function Bot:SubscribeTakeDamage()
  ...
  self.Character:Subscribe("TakeDamage", function(char, damage, _, _, _, instigator)
    if instigator and instigator:IsA(Player) then
      instigator = instigator:GetControlledCharacter()
    end
    if bot:GetPlayer() == nil then
      if bot and instigator and instigator:IsA(Character) and instigator:GetHealth() > 0 then
...
function Bot:Surroundings()
  local weapon_aware = Trigger(Vector(0, 0, -100000), Rotator(), Vector(self.WeaponAwareSize), TriggerType.Sphere, false,
    Color(1, 1, 1))
...
function GetEnemyHP(level)
  return math.random(math.ceil((0.1 * level ^ 3 + 0.11 * level ^ 2 + 1.05 * level ^ 0.5 + level) + 80),
    math.ceil((0.2 * level ^ 3 + 0.12 * level ^ 2 + 1.1 * level ^ 0.7 + level) + 90))
end

function Bot.new(location, team, level, hp, shield, id)
  local self = setmetatable({}, Bot)

  self.Character = Character(location, Rotator(0), "nanos-world::SK_Mannequin")
  self.Team = team or math.random(2, 10000000)
  self.Character:SetTeam(math.random(2, 10000000))
  self.Movement = nil
