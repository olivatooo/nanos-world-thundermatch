function EnergyBall(spawn, start_location, end_location, custom_damage)
  ...
  trigger:Subscribe("BeginOverlap", function(trigger, actor_triggering)
    if actor_triggering:IsA(StaticMesh) and actor_triggering and trigger and trigger:IsValid() and actor_triggering:IsValid() then
...
function BotCombatSniper(self)
  ...
  if bot and bot:IsValid() then
    ...
    if enemy and enemy:IsValid() and enemy:GetHealth() > 0 and enemy:IsA(Character) then
...
function Kamikaze(self)
  ...
  character:Subscribe("Death", function()
    local explosion = Grenade(character:GetLocation(), Rotator(0, 90, 90), "nanos-world::SM_Grenade_G67",
      "nanos-world::P_Explosion_Water", "nanos-world::A_Explosion_Large")
...
function Bombardier(self)
  ...
  Timer.SetInterval(function()
    ...
    if bot and bot:IsValid() then
      if enemy and enemy:IsValid() and enemy:GetHealth() > 0 and enemy:IsA(Character) then
        local enemy_location = enemy:GetLocation()
        EnergyBall(bot:GetLocation() + Vector(0, 0, 1000), enemy:GetLocation(), enemy:GetLocation(),
          math.ceil(bot:GetHealth() * 0.01))
      else
        return false
      end
    end
